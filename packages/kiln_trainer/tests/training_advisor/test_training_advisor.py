...
def test_polling_advisor_with_mocked_sdk_returns_one_line_observation():
    ...
    from kiln_trainer import events as ke
    ...
    # Audit H6: emissions now go through ``events.emit`` which writes
    # through ``events.sys.stdout`` rather than ``training_advisor``'s
    # own ``sys``. Patch both so stdin reads (still on ta.sys) and
    # stdout writes (now on ke.sys) are intercepted.
    with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "sk-ant-test"}, clear=False), \
         patch("anthropic.Anthropic", return_value=fake_client), \
         patch.object(ta, "sys", MagicMock(stdin=MagicMock(read=lambda: json.dumps({
             "samples": [{"prompt": "morning", "completion": "Coffee at 6:30."}],
             "loss_trajectory": [2.5, 2.3, 2.1, 1.9],
             "iter": 100, "iter_total": 200,
         })), argv=["x"], stdout=fake_stdout)), \
         patch.object(ke, "sys", MagicMock(stdout=fake_stdout)):

...
## 10. DESIGN.md as single source of truth for design tokens; Swift projection is hand-crafted

- **Date:** 2026-04-23
- **Context:** Phase 1 of the UI track introduced `/DESIGN.md` in Google Labs' `design.md` format (YAML frontmatter + markdown prose, lintable via `@google/design.md`) as the frozen source of truth for Kiln's color, type, spacing, and radius tokens. `apps/Kiln/Sources/DesignSystem.swift` had existed since M0, but its token names (`accent`, `accentWash`, `Space.s/m/l`, `Radius.control/card/modal`) no longer matched DESIGN.md, and some values collided at the same symbolic name — old Swift `Space.m` = 24pt, DESIGN.md `space.m` = 16pt. Phase 2 needed to decide how the two files stay in sync.
- **Options considered:**
  - **(a)** Keep `DesignSystem.swift` as the authoritative source and treat DESIGN.md as documentation. Simple, but external tooling (Figma plugins, docs sites, any design-system consumer that isn't Swift) has no machine-readable entry point, and the `design.md` lint / DTCG export stops being meaningful.
  - **(b)** Generate `DesignSystem.swift` from `docs/design/tokens.dtcg.json`. Mechanically tidy, but the Swift projection benefits from idioms that don't round-trip cleanly through DTCG — `Color(nsColor: .controlBackgroundColor)` for `surface-sunken`, `SwiftUI.Font.system(.body, design: .default).weight(.medium).monospacedDigit()` for the `numeric` token, `@available` gating, SwiftUI's semantic-layer bridging (`.primary`, `.secondary`, `.tertiary`, `.regularMaterial`). A generator would either produce less idiomatic Swift or need per-token escape hatches.
  - **(c)** Hand-craft `DesignSystem.swift` as a Swift *projection* of DESIGN.md. DESIGN.md is the frozen source of truth (lint-gated on every commit when staged); the Swift file stays small (~100 lines), idiomatic, and reviewable; the DTCG export feeds external tooling.
- **Choice:** (c). `/DESIGN.md` is normative. Token changes land in DESIGN.md first, `make design-lint` passes (the pre-commit hook gates this when DESIGN.md is staged), then `DesignSystem.swift` is updated to match. `DesignSystem.swift` opens with a comment block spelling out this discipline so the direction is obvious at the first site of a would-be drift.
